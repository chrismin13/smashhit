...
class DamageResolver_1_8_R1 implements DamageResolver {

	private boolean USE_CRITS, OLD_CRITS;

	DamageResolver_1_8_R1(boolean useCrits, boolean oldCrits) {
		USE_CRITS = useCrits;
		OLD_CRITS = oldCrits;
	}
	...
	public double getDamage(Player damager, Damageable entity) {
		double damage = ((CraftPlayer)damager).getHandle().getAttributeInstance(GenericAttributes.e).getValue();
		if(USE_CRITS
