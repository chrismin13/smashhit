...
class DamageResolver_1_9_R1 implements DamageResolver {
	...
	public double getDamage(Player damager, Damageable entity) {
		double damage = ((CraftPlayer) damager).getHandle().getAttributeInstance(GenericAttributes.ATTACK_DAMAGE)
				.getValue();
		if (USE_CRITS && !((Entity) damager).isOnGround() && damager.getVelocity().getY() < 0 && OLD_CRITS
				|| !damager.isSprinting())
			damage *= 1.5;
